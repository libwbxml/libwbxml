...
do
  ...
  if [ `basename $i` != 'ddf' ]; then
  	...
  	TESTLIST=$(find $i -maxdepth 1 -type f \( -name "*.ddf" -or -name "*.xml" \) -print | sort )
  fi
  if [ " $3" != " " ]
  then
	COUNTER=0
  ...
  do
    ...
    then
	echo UNSUPPORTED
    else if [ `basename $j` = "syncml-012.xml" -o  `basename $j` = "syncml-013.xml" ]
    then
	# SyncML CDATA fix makes comparing sometimes impossible
	echo CDATA_ENCAPSULATION
    else if [ `basename $j` = "wv-021.xml" -o  `basename $j` = "wv-022.xml" -o `basename $j` = "wv-023.xml" -o `basename $j` = "wv-024.xml" ]
    then
	# WV tests which test hexadecimal numbers can only executed
	# because libwbxml always outputs decimal numbers
	echo HEX_CONVERSION
    else
